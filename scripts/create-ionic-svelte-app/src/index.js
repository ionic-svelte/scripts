...
async function main() {
  ...
  if (!opts?.quiet) {
    ...
    if (options.ionicons) {
      ...
      console.log(
        cyan("  https://ionic-svelte.firebaseapp.com/components/Icon")
      );
    }
    ...
    if (options.capacitor) {
      console.log(
        `\nCapacitor configuration - see: ${bold(
          cyan("capacitor.config.json|ts")
        )}`
      );
      console.log(`  App name ${bold(cyan(opts.name))}`);
      console.log(`  Package name ${bold(cyan(opts.name + ".ionic.io"))}`);
      console.log(
        `  Vite dev server url ${bold(cyan("http://192.168.137.1:5173/"))}`
      );
    }
    ...
    if (options.capacitor && opts.types != "typescript") {
      console.log(
        `\nWant HMR in Capacitor dev mode? Rename ${bold(
          cyan("_server")
        )} to ${bold(cyan("server"))} in ${bold(cyan("capacitor.config.json"))}`
      );
    }
    if (options.capacitor && opts.types == "typescript") {
      console.log(
        `\nUse the ${bold(cyan("-hmr"))} flag after your ${bold(
          cyan("npx cap run/open/sync")
        )} commands to use HMR together with ${bold(cyan("npm run dev"))}`
      );
    }

    console.log(
      `\nHint: Make your app offline and near native by turning it into a progressive web app - see ${cyan(
        " https://github.com/vite-pwa/sveltekit"
      )}`
    );

    console.log(
      `\nStuck? Visit us at Ionic's discord ${cyan(
        "https://discordapp.com/channels/520266681499779082/1049388501629681675"
      )}`
    );
...
`;
  ...
  if (response.twplugins != undefined)
    Object.keys(response.twplugins).forEach(
      (index) => (opts[response.twplugins[index]] = true)
    );
